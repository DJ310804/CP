...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        int numerator = (mod_pow(x, k + 1, MOD) - 1 + MOD) % MOD;
        int denominator = mod_pow(x - 1, MOD - 2, MOD);
        b = (b * numerator % MOD * denominator % MOD) % MOD;

        int exp = (k * (k + 1) / 2) % (MOD - 1); 
        c = mod_pow(c, k + 1, MOD);
        c = (c * mod_pow(mod_pow(x, exp, MOD), total_exponents, MOD)) % MOD;
