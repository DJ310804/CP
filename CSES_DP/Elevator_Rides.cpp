...
void solve() {
    ...
    vpii dp(1 << n, make_pair(LLONG_MAX, 0)); // {rides, current weight}
    dp[0] = make_pair(1, 0); 
    ...
    for (int mask = 1; mask < (1 << n); mask++) {
        for (int i = 0; i < n; i++) {
            if (mask & (1 << i)) { 
                ...
                int prev_rides = dp[prev_mask].first;
                int prev_weight = dp[prev_mask].second;

                if (prev_weight + w[i] <= x) {
                    dp[mask] = min(dp[mask], make_pair(prev_rides, prev_weight + w[i]));
                } else {
                    dp[mask] = min(dp[mask], make_pair(prev_rides + 1ll, w[i]));
                }
            }
        }
    }
